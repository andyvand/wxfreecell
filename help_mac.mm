...
@interface NSHelpManager (wxFreeCellPrivate)
...
extern "C" void ShowMacHelpBook(const char *anchor)
{
    @autoreleasepool {
        ...
        if ([mgr respondsToSelector:@selector(registerBooksInBundle:error:)]) {
            ...
            if (!registered) {
                NSLog(@"wxFreeCell: registerBooksInBundle failed: %@", err);
            }
        }
        ...
        if (url) {
            ...
            @try { [task launch]; }
            @catch (NSException *e) {
                NSLog(@"wxFreeCell: launching Help Viewer failed: %@", e);
                if (book) [mgr openHelpAnchor:@"wxfreecell" inBook:book];
                else [[NSApplication sharedApplication] showHelp:nil];
            }
        } else if (book) {
            [mgr openHelpAnchor:@"wxfreecell" inBook:book];
        } else {
